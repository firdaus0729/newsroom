...
export async function getStories(params = {}) {
  const q = new URLSearchParams(params).toString();
  const res = await fetch(`${API_BASE}/api/stories?${q}`, { headers: authHeaders() });
  const data = await res.json().catch(() => ({}));
...
export async function getStory(id) {
  const res = await fetch(`${API_BASE}/api/stories/${id}`, { headers: authHeaders() });
  const data = await res.json().catch(() => ({}));
...
export async function retryStoryJob(storyId, jobId) {
  const res = await fetch(`${API_BASE}/api/stories/${storyId}/retry-job`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
...
export async function approveClip(storyId, clipId, note = '') {
  const res = await fetch(`${API_BASE}/api/stories/${storyId}/approve-clip`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
    body: JSON.stringify({ clip_id: clipId, note }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(data.error || 'Failed to approve clip');
  return data;
}

export async function rejectClip(storyId, clipId, note = '') {
  const res = await fetch(`${API_BASE}/api/stories/${storyId}/reject-clip`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...authHeaders() },
    body: JSON.stringify({ clip_id: clipId, note }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(data.error || 'Failed to reject clip');
  return data;
}

export async function getQueueHealth() {
  const res = await fetch(`${API_BASE}/api/jobs/queue-health`, { headers: authHeaders() });
  const data = await res.json().catch(() => ({}));
